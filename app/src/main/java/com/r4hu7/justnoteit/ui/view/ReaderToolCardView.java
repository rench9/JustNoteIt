package com.r4hu7.justnoteit.ui.view;
...
public class ReaderToolCardView extends MaterialCardView implements View.OnClickListener, SeekBar.OnSeekBarChangeListener {
    ...
    public interface ReaderToolChangeListener {
    ...
    private WeakReference<ReaderToolChangeListener> readerToolChangeListener;
    ...
    public void addReaderToolChangeListener(@NonNull ReaderToolChangeListener readerToolChangeListener) {
        this.readerToolChangeListener = new WeakReference<>(readerToolChangeListener);
    }

    @Override
    public void onClick(View view) {
        if (readerToolChangeListener == null || readerToolChangeListener.get() == null)
            return;
        switch (view.getId()) {
            case R.id.btnSmallChar:
                if (selectedFontSize > 0) {
                    readerToolChangeListener.get().OnFontSizeChange(fonts[selectedFontSize -= 1]);
            ...
            case R.id.btnNightMode:
                readerToolChangeListener.get().OnNightModeChange(nightMode = !nightMode);
                break;
            case R.id.btnBrightness:
//                switchToBrightnessTool();
                if (readerToolChangeListener != null && readerToolChangeListener.get() != null)
                    readerToolChangeListener.get().OnSystemPermissionRequired();
                else throw new NullPointerException("ReaderToolChangeListener is not defined");
                break;
    ...
    public void switchToBrightnessTool() {
        ...
        try {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N)
                sbBrightness.setProgress(Settings.System.getInt(getContext().getContentResolver(), Settings.System.SCREEN_BRIGHTNESS), true);
            else
                sbBrightness.setProgress(Settings.System.getInt(getContext().getContentResolver(), Settings.System.SCREEN_BRIGHTNESS));
